    ...
    : m_run(1)
{
    ...
    /* Initialize token */
    m_token = "NNSXS.CI4KYAD4WEVQUQHTBI77DPQQ2WN2IROIKWRCLDA.TEXTNWIJMJ4Z5LSGJVBBKJ72QAL3S3GAQDHFTTOR4GUDZI2TCHWQ";    
...
TTNHelper::CloseConnection(int signal) const
{
    ...
    for(int j = 0; j<m_session.nGateway;j++){
     ...
     for (int i = 0; i < m_session.nDevices; i++) {
        ...
        if (DELETE("/api/v3/ns/applications/" + str(m_session.appId)+"/devices/device-"+ std::to_string((unsigned)n_ed), reply) == EXIT_FAILURE)
        {
            NS_LOG_ERROR("Unable to unregister device from NS, got reply: " << reply);
        }
        if (DELETE("/api/v3/as/applications/" + str(m_session.appId)+"/devices/device-"+std::to_string((unsigned)n_ed), reply) == EXIT_FAILURE)
        {
            NS_LOG_ERROR("Unable to unregister device from AS, got reply: " << reply);
        }
        if (DELETE("/api/v3/applications/" + str(m_session.appId)+"/devices/device-"+std::to_string((unsigned)n_ed), reply) == EXIT_FAILURE)
        {
...
TTNHelper::NewApplication(const str& name)
{
    ...
    str payload = "{"
                  "  \"application\": {                                                             "
                  "   \"ids\": {                                                                    "
                            "\"application_id\" : \""+ m_session.appId+ "\"   "
                  "},                                                                               "
                  "   \"name\": \"eloratest\",                                                      "
...
TTNHelper::NewDevice(Ptr<Node> node) const
{
    ...
    "\"end_device\": {"
    "\"ids\": {"
      "\"device_id\": \"device-" + std::to_string((unsigned)id)+"\","
      "\"dev_eui\": \""+  str(eui)+"\""
    "},"
    "\"join_server_address\": \"localhost\","
    "\"network_server_address\": \"localhost\","
  ...
  "\"field_mask\": {"
    ...
    str reply;
    if (POST("/api/v3/applications/"+ str(m_session.appId) + "/devices", payload, reply) == EXIT_FAILURE)
    {
    ...
    payload = "{"
            "\"end_device\": {"
                    ...
                    "\"session\": {"
                        " \"keys\": {"
                            "\"f_nwk_s_int_key\":{"
                                "\"key\": \"" + m_session.netKey+"\""
                            "}"
                        "},"
                        "\"dev_addr\":\""+str(devAddr)+"\""
                    "},"
                    "\"mac_settings\": {"
                        "\"resets_f_cnt\": true,"
    ...
    if (PUT("/api/v3/ns/applications/"+ str(m_session.appId) + "/devices/device-" + std::to_string((unsigned)id), payload, reply) == EXIT_FAILURE)
    {
        NS_FATAL_ERROR("Unable to activate device device-" << std::to_string((unsigned)id) << ", reply: " << reply);
    }
    ...
    if (PUT("/api/v3/as/applications/"+ str(m_session.appId) + "/devices/device-" +  std::to_string((unsigned)id), payload, reply) == EXIT_FAILURE)
    {
        NS_FATAL_ERROR("Unable to activate device device-" << std::to_string((unsigned)id) << ", reply: " << reply);
    }
...
TTNHelper::NewGateway(Ptr<Node> node) const
{
    ...
    str payload = "{"
                  ...
                  "         \"ids\": {                                                             "
                  "         \"eui\": \"" + str(eui) + "\",                                     "
                  "         \"gateway_id\":\"gw-" + std::to_string((unsigned)id)  + "\"          "
                  "                  },                                                            "
                  "    \"name\": \"gw-" + std::to_string((unsigned)id) +        "\",          "
...
size_t
TTNHelper::StreamWriteCallback(char* buffer,
                                      size_t size,
                                      size_t nitems,
                                      std::ostream* stream)
{
